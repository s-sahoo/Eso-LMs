...
def attention_inner_heads_flex(qkv, num_heads):
    bs, width, length = qkv.shape
    ch = width // (3 * num_heads)
    q, k, v = qkv.chunk(3, dim=1)
    q = q.view(bs, num_heads, ch, length).transpose(2, 3)  # (B, H, T, C)
...
class TestAttentionInnerHeads(unittest.TestCase):
    def setUp(self):
    ...
    def test_attention_inner_heads(self):
